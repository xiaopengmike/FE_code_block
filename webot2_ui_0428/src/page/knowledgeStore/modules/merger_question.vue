<template>
  <div class="_edit_question">
    <my-modals v-model="currentValue" :cover="cover">
        ...
        <el-form :model="question" :rules="question_rules"  ref="question" class="small_modals">
          <div v-show="AUX.tab_index==0">
            <el-form-item label="标准问题" prop="question" >
              <el-input v-model="question.question" placeholder="填写访客可能出现的问题"></el-input>
            </el-form-item>
            <el-form-item label="分类" prop="tag1" class="question-tag">
              <el-select v-model="question.tag1" placeholder="" @visible-change="selectTag1">
                <el-option v-for="op in tags" v-if="op.id!==''" :value="op.id" :label="op.name"></el-option>
              </el-select>
            ...
            <el-form-item  v-for="(val,index) in question.similar_question"  :prop="'similar_question.'+index"
                           :label="index==0?'相似问题':''" :rules="question_rules.similar_question" class="similar-question ">
              <el-col :span="22" v-show="question.similar_question[index]">
                <el-input v-model="question.similar_question[index]"  ></el-input>
              </el-col>
              <el-col :span="1" :offset="1">
                <i class="el-icon-close pointer" @click="question.similar_question.splice(index,1)" v-show="question.similar_question[index]"></i>
              </el-col>
            </el-form-item>
            <span class="pointer add-similar" @click="question.similar_question.push(' ')" >
              <i class="fa fa-plus"></i>&thinsp;添加相似问题</span>
            <el-tabs type="card" v-model="questionTabs">
              <el-tab-pane label="普通答案" name="normal_answer" class="normal-answer">
                ...
                <el-form-item v-for=" (val,index) in question.answer" :prop="'answer.'+index" :rules="question_rules.answer">
                  <el-col :span="22">
                    <el-input type="textarea" v-model="question.answer[index]" v-show="question.answer[index]!=undefined" placeholder="请输入标准答案"></el-input>
                  </el-col>
                  <el-col :span="1" :offset="1">
                    <i v-if="question.answer.length!=1" v-show="question.answer[index]!=undefined" class="el-icon-close pointer" @click="question.answer.splice(index,1)"></i>
                  </el-col>
                </el-form-item>
                <span class="pointer add-similar" @click="question.answer.push('')" >
                  <i class="fa fa-plus"></i>&thinsp;添加标准回答</span>
              </el-tab-pane>
      ...
      <div slot="foot">
        <button class="my-btn my-btn-transparent " @click=clearForm()>&emsp;取消&emsp;</button>&emsp;&emsp;
...
<script>
  ...
  export default {
    ...
    // function(){
    // 	console.log("rich_answer", this.question.rich_answer.length)
    // 	if(this.question.rich_answer&&this.question.rich_answer.length>0){
    // 		this.questionTabs = "rich_answer";
    // 	}else{
    // 		this.questionTabs = "normal_answer";
    // 	}
    // },
    created(){
    ...
    computed: {
      tag2List: function() {
        ...
        for(var i = 0; i < this.tags.length; i++) {
          if(this.tags[i].id == this.question.tag1) {
            list = this.tags[i].child;
            break;
          }
        }
        return list;
...
<style lang="scss">
  ...
  ._edit_question{
    .rich-answer{
      ...
      .normal-answer .title,.rich-answer .title{
        width: 100px; text-align: right; padding: 11px 12px 11px 0; float: left; font-size: 14px;
        display: none;
        &:before {content: '*'; color: #ff4949; margin-right: 4px;}
      }
      .rich-answer-show{
        .media {
          border: 1px solid #bfcbd9; height: 70px; border-radius: 4px; padding: 5px; background: #f6f6f6; font-size: 14px; position: relative;
          a,button{position: absolute; right: 20px; top: 0; bottom: 0;}
        }
        .media-text {
          position: relative;
          button{position: absolute; right: 20px; top: 0; bottom: 0;}
        }
      }
  ...
  .rich-answer-show .media-audio p{line-height: 26px; height: 26px;}
  ...
  .delete-media {position: absolute; right: 2px; top: 0; bottom: 0; margin: auto; height: 18.5px;}
  .media button span,
