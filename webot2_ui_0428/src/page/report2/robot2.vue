<template>
  <div class="report">
    <h1 class="page_title">
      机器人&nbsp;<word-explain keys="工作报表-总览-在线服务数据"></word-explain>
    ...
    <h1 class="page_title_rewrite no_underLine_tittle">
      热点问题咨询&nbsp;<word-explain keys="工作报表-总览-在线服务数据"></word-explain>
    </h1>
    <div>
      <div class="clearfix">
        <div class="col-xs-12">
          <div class="part">
            <div class="large_canvas">
              <chart :options="热点问题咨询_option" :auto-resize="true"></chart>
            </div>
          </div>
        </div>
      </div>
      ...
      searchs() {
        ...
        v.$api.ajax(v.$uri.company.index, 'get', search)
          .then(result => {
            ...
            for (let i = 0; i < 5; i++) {
              v.首次响应时间_barOption.series[1].data[i] = !首次响应时间[i] ? 0 : (首次响应时间[i] / (首次响应个数 || 1) * 100).toAccuracy(2);
              v.首次响应时间_barOption.yAxis[1].data[i] = v.首次响应时间_barOption.series[1].data[i] + '%';
              v.响应时间_barOption.series[1].data[i] = !响应时间[i] ? 0 : (响应时间[i] / (响应个数 || 1) * 100).toAccuracy(2);
              v.响应时间_barOption.yAxis[1].data[i] = v.响应时间_barOption.series[1].data[i] + '%';
              v.会话时长_barOption.series[1].data[i] = !会话时长[i] ? 0 : (会话时长[i] / (人工会话量 || 1) * 100).toAccuracy(2);
