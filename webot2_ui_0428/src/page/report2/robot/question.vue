<template>
  <div class="report report_robot_question">
    <h1 class="page_title noBorder">
      热点问题统计&nbsp;<word-explain keys="工作报表-机器人-热点问题统计"></word-explain>
      ...
      <div class="table4 simulate_table">
        <ul class="thead">
          <li class="tr">
            ...
            <div class="th">匹配次数&nbsp;<sort akey="match_count" :active="page.sort_filed=='match_count'" :status="page.sort" @click="changeFilterSort"></sort></div>
            <div class="th">匹配率&nbsp;<sort akey="match_ratio" :active="page.sort_filed=='match_ratio'" :status="page.sort" @click="changeFilterSort"></sort></div>
          </li>
        </ul>
        <ul class="tbody">
          <li class="tr" v-for="(item,i) in list">
            <div class="td" :title="item.热点问题">{{item.热点问题||0
              }}</div>
            {{item.热点问题
            }}
            <div class="td" :title="item.问题分类">{{item.问题分类||0}}</div>
            <div class="td" :title="item.匹配次数">{{item.匹配次数||0
              }}</div>
  ...
  export default {
    ...
    methods: {
      download(){
        ...
        var url = '/api/statistic/export/hot_question_statistics?cid=1&'
      ...
      searchs(index=0) {
        ...
        v.page.start = start;
        v.page.end = end;
        v.$api.ajax('/statistic/hot_question_statistics', 'get', v.page)
          .then(data => {
          v.allData=data;
          v.list=data.list;
