...
export default {
	...
	watch: {
		value(newValue) {    console.log(this.url)
	...
	methods: {
		...
		startCrop() {
			...
			img.onload = function() {
				...
				scale.naturalWidth = nWidth*0.5;
				scale.naturalHeight = nHeight*0.5;
				that.sourceImg = img;
				that.createImg();
				that.setStep(2);
			};
        ...
        createImg(e) {
            ...
            // 将透明区域设置为白色底边
            if(that.tempImgFormat!=undefined&&that.tempImgFormat.indexOf('jpg')!=-1){
            	ctx.fillStyle = "#fff";
