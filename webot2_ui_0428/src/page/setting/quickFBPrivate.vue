<template>
	<div id="Fast-reply">
		...
		<div class="list">
			...
			<div v-if="list.length>0" class="pagination_wrap">
				<el-pagination class="my_pagination" :total="list.length" :current-page.sync="page.page" :page-size="page.size" @current-change="changePage" layout="total, prev, pager, next, jumper"></el-pagination>
			</div>
		</div>
		...
		<!--<el-dialog title="编辑回复" v-model="show_edit" size="tiny" >
		  	<span>
		  		<form class="form-horizontal">
					<div class="form-group input-group-sm">
						<label class="col-sm-3">分类</label>
						<div class="col-sm-8">
								<select v-model="reply.tag" class="form-control">
									<option v-for="(op,i) in classifys" v-if="i>0" :value="op.name">{{op.name}}</option>
								</select>
						</div>
					</div>
					<div class=" form-group">
						<label class="col-sm-3">快捷词</label>
						<div class="col-sm-8">
							<div class=' input-group'>
								<span class="input-group-addon">#</span>
				            	<input type="text" v-model="reply.qKey" class="form-control" placeholder="">
							</div>
			            </div>
			        </div>
					<div class="form-group">
						<label for="standardAnswer" class="col-sm-3">回复内容</label>
						<div class="col-sm-8">
							<textarea class="form-control" rows="4" placeholder="请输入标准答案" v-model="reply.qContent" >
							</textarea>
						</div>
					</div>
				</form>
...
<script>
	...
	export default{
		...
		methods:{
			searchs(){
				let v = this;
				v.$api.ajax(v.$uri.setting.快捷回复.personal,'get')
				.then(data=>{
					if(data.quickFBUser && data.quickFBUser.tag!=undefined){
						for(let i in data.quickFBUser.tag){
							v.classifys.push({id:i,name:i,copy:i,edit:false});
						}
						v.resource.copy = v.$api.deepCopy(data.quickFBUser.tag);
						v.resource.source = v.$api.deepCopy(data.quickFBUser.tag);
						v.getList();
				    }
				});
			},
			...
			update(log){
				...
				return new Promise((resolve,reject)=>{
					v.$api.ajax('/company/setting/quickFBUser','put',{tag:v.resource.source,action:log})
					.then(data=>{
						v.$message({
							message:'提交成功',
							type:'success'
			...
			edit(reply){
				...
				// console.info("resource",v.resource)
				if(reply){
